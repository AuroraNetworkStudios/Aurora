...
import gg.auroramc.auroralib.expansions.region.*;
...
public class FileRegionStorage implements RegionStorage {
    ...
    public void loadRegion(Region region) {
        ...
        var path = AuroraLib.getInstance().getDataFolder() + "/regiondata/" + worldName + "/r." + regionX + "." + regionZ + ".txt";
        ...
        synchronized (lockCache.getUnchecked(path)) {
            ...
            if (file.exists()) {
                try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                    ...
                    if (file.delete()) {
                        AuroraLib.logger().warning("Deleted " + file.getName() + " because it was corrupted, this won't affect anything.");
                    }
                }
            }
        }
    ...
    public void saveRegion(Region region) {
        var path = AuroraLib.getInstance().getDataFolder() + "/regiondata/" + region.getWorldName() + "/r." + region.getX() + "." + region.getZ() + ".txt";
    ...
    public void deleteRegionsInWorld(String worldName) {
        Path filePath = Paths.get(AuroraLib.getInstance().getDataFolder() + "/regiondata/" + worldName);
        File file = filePath.toFile();
