...
import gg.auroramc.auroralib.expansions.region.BlockPosition;
import gg.auroramc.auroralib.expansions.region.ChunkCoordinate;
import gg.auroramc.auroralib.expansions.region.ChunkData;
...
public class SqliteRegionStorage implements RegionStorage {
    private final String DB_URL = "jdbc:sqlite:" + AuroraLib.getInstance().getDataFolder() + "/regiondata.db";
    ...
    public SqliteRegionStorage() {
        var tables = new String(AuroraLib.getInstance().getResource("database/region_schema.sql").readAllBytes(), StandardCharsets.UTF_8).split(";");
